...
summarize_group <- function(dat, group_name) {
  out <- list(
    ...
    "Race" = "",
    "White" = sprintf("%d (%.1f%%)",
                      sum(dat$Race_Ethnicity == 1, na.rm = TRUE),
                      100 * mean(dat$Race_Ethnicity == 1, na.rm = TRUE)),
    "Asian/PI" = sprintf("%d (%.1f%%)",
                         sum(dat$Race_Ethnicity == 2, na.rm = TRUE),
                         100 * mean(dat$Race_Ethnicity == 2, na.rm = TRUE)),
    "Black" = sprintf("%d (%.1f%%)",
                      sum(dat$Race_Ethnicity == 4, na.rm = TRUE),
                      100 * mean(dat$Race_Ethnicity == 4, na.rm = TRUE)),
    "Mixed or Other" = sprintf("%d (%.1f%%)",
                               sum(dat$Race_Ethnicity == 5, na.rm = TRUE),
                               100 * mean(dat$Race_Ethnicity == 5, na.rm = TRUE)),
    
    "Previous Live Birth" = "",
    "Yes" = sprintf("%d (%.1f%%)",
                    sum(dat$Parous == 1, na.rm = TRUE),
                    100 * mean(dat$Parous == 1, na.rm = TRUE)),
    "No" = sprintf("%d (%.1f%%)",
                   sum(dat$Parous == 0, na.rm = TRUE),
                   100 * mean(dat$Parous == 0, na.rm = TRUE))
  )
  
  tibble(
    Characteristic = names(out),
    !!group_name := unlist(out)
  )
}

# --- Summaries ---
cases_summary <- summarize_group(
  cases,
  sprintf("Early Pregnancy Loss (N = %d)", nrow(cases))
)

controls_summary <- summarize_group(
  controls,
  sprintf("Ongoing Pregnancy (N = %d)", nrow(controls))
)

table1 <- full_join(cases_summary, controls_summary, by = "Characteristic")

# --- Hardcoded additional rows ---
extra_rows <- tibble(
  Characteristic = c(
    "Karyotype",
    "Euploid",
    "Aneuploid",
    "Contamination / Insufficient / Untested",
    "Loss Group",
    "Recurrent Pregnancy Loss (RPL)",
    "Early Pregnancy Loss (EPL)",
    "1st EPL"
  ),
  `Early Pregnancy Loss (N = 46)` = c(
    "",
    "21",
    "18",
    "7",
...
# --- Append rows ---
table1 <- bind_rows(table1, extra_rows)

# --- Identify section headers ---
section_rows <- grep(
  "Age|BMI|Mode of Conception|Race|Previous Live Birth|Karyotype|Loss Group",
