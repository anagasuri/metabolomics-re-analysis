...
out_dir = "/Users/amritanagasuri/Desktop/new Q-TOF Analysis/4. Cluster/1. All-sig/U-test/U-test new/Stats-U-Test.py_output"
os.makedirs(out_dir, exist_ok=True)

# Load data
# df = pd.read_csv('/Users/amritanagasuri/Desktop/new Q-TOF Analysis/4. Cluster/1. All-sig/regress output/Pos_removebatcheffect_corrected_NEW.csv', index_col=0)  # index is Alignment ID
df = pd.read_csv('/Users/amritanagasuri/Desktop/new Q-TOF Analysis/4. Cluster/1. All-sig/regress output/Neg_removebatcheffect_corrected_NEW.csv', index_col=0)
...
mts['(I/m)Q - p'] = mts['(I/m)Q'] - mts['p_value']
mts = mts.sort_values(by='(I/m)Q - p', ascending =False)
mts['BH_sig'] = np.where(mts['(I/m)Q - p'] < 0, 0, 1)

mts_2 = mts[mts['BH_sig'] == 1]
mts_2 = mts_2[mts_2['p_value'] < 0.01]

# Save
# mts_2.to_csv(os.path.join(out_dir, 'Aneuploid_PosPVFStats2d.csv'), index=False)
# mts.to_csv(os.path.join(out_dir, 'Aneuploid_PosPVFStats2d_FULL.csv'), index=False)
# results.to_csv(os.path.join(out_dir, 'Aneuploid_PosPVFStats2d_RESULTS_ONLY.csv'), index=False)

mts_2.to_csv(os.path.join(out_dir, 'Aneuploid_NegPVFStats2d.csv'), index=False)
mts.to_csv(os.path.join(out_dir, 'Aneuploid_NegPVFStats2d_FULL.csv'), index=False)
results.to_csv(os.path.join(out_dir, 'Aneuploid_NegPVFStats2d_RESULTS_ONLY.csv'), index=False)
